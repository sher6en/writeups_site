...
As for `genRandomNumber`, it's just a wrapper to a function called `SystemFunction036` (from `advapi.dll`). Looking it up in google says that it calls just `RtlGenRandom`, which generates and writes `RandomBufferLength` random bytes starting at address `RandomBuffer`:
![genRandomNumber function](genRandomNumber.png)
##### Encryption logic
